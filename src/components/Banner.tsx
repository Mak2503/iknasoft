const Banner = () => {
  return (
    <div className="bg-[#F4835F] lg:py-4 py-6 px-10 flex lg:flex-row flex-col items-center gap-5 justify-center">
      <span className="text-xs text-white text-center font-normal">
        Usage of Illegal Property Images to upload content showing area in non
        standard/Illegal units is prohibited
      </span>
      <button className="py-1.5 px-3 bg-white text-[10px] text-black rounded-full font-semibold">
        Learn More
      </button>
    </div>
  );
};

export default Banner;
